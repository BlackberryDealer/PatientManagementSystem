...
/// (non-cancelled) appointment for the same doctor OR the same room.
///
/// ## Overlap logic
/// Two intervals [A_start, A_end) and [B_start, B_end) overlap iff:
///   A_start < B_end AND A_end > B_start
///
/// Doctor and room are independent resources: a doctor is busy no matter
/// which room the existing visit sits in, and a room is occupied no matter
/// whose doctor booked it — so either match alone is a conflict. (Requiring
/// both to match let a doctor be double-booked across two rooms.)
